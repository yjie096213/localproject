...
public class DeviceConfigService {
    ...
    public ResultValue insertDeviceConfig(DeviceConfig deviceConfig) {
        DeviceConfig d = checkIsExists(deviceConfig);
        if(null != d){
            return ResultValue.error(2000,"该设备已存在");
        }
    ...
    public ResultValue updateDeviceConfig(DeviceConfig deviceConfig) {
        DeviceConfig d = checkIsExists(deviceConfig);
        if(null != d && !d.getUuid().equals(deviceConfig.getUuid())){
            return ResultValue.error(2000,"该设备已存在");
        }
    ...
    public DeviceConfig checkIsExists(DeviceConfig deviceConfig) {
        DeviceConfig d = deviceConfigDao.checkIsExists(deviceConfig);
        return d;
    }
}
